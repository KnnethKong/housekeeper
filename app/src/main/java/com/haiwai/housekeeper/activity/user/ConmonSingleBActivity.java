...
public class ConmonSingleBActivity extends AppCompatActivity {
                ...
                if (i == 3) {
                    ...
                    multi_common_view.getIv_th_red().setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View view) {
                            ToastUtil.longToast(ConmonSingleBActivity.this, "弹出计算表格");
                        }
                    });
                } else {
            ...
            if (view == top_single_bar.getBackView()) {
                ...
                if ("3".equals(type)) {//庭院管理--含有多选
                    ...
                    if (!TextUtils.isEmpty(et_single_input_price.getText().toString().trim())) {
                        SPUtils.saveString(ConmonSingleBActivity.this, "want_money", et_single_input_price.getText().toString().trim());
                    }
                ...
                if (sigle_common_view.getSelectItemId().equals("") && multi_common_view.getSelectItemId().equals("")) {
                    ToastUtil.shortToast(ConmonSingleBActivity.this, getString(R.string.select_at_last_one));
                    return;
                }
                if ("3".equals(type)) {
                    IMKitService.weekMap.put("step" + 2, zyEntity);
                } else {
                    IMKitService.weekMap.put("step" + step, zyEntity);
                }
                if ("3".equals(type)) {
                    Log.i("WnInfomation", IMKitService.weekMap.get("step1").getDaList().get(0).getStrId() + "");
                    SPUtils.saveBoolean(ConmonSingleBActivity.this, "isThird", true);
                    if ("28".equals(IMKitService.weekMap.get("step1").getDaList().get(0).getStrId()) || "29".equals(IMKitService.weekMap.get("step1").getDaList().get(0).getStrId())) {
                        Intent intent = new Intent(ConmonSingleBActivity.this, ConmonSingleCActivity.class);
                        intent.putExtra("type", type);
                        startActivity(intent);
                    } else {
                        SPUtils.saveBoolean(ConmonSingleBActivity.this, "isThird", false);
                        Intent intent = new Intent(ConmonSingleBActivity.this, ConmonSingleDActivity.class);
                        intent.putExtra("type", type);
                        startActivity(intent);
                    }
                } else {
                ...
                for (int i = 1; i < VipWDUtils.mCount; i++) {
                    List<ZYEntity.DaBean> daList = new ArrayList<>();
                    ZYEntity zyEntity = new ZYEntity();
                ...
                final Intent intent = new Intent(ConmonSingleBActivity.this, OutLineActivity.class);
                Bundle bundle = new Bundle();
                bundle.putSerializable("map", sMap);
                intent.putExtras(bundle);
                intent.putExtra("type", type);

                CustomDialog.Builder dialog = new CustomDialog.Builder(ConmonSingleBActivity.this);
                ...
                if (AppGlobal.getInstance().getIsHome()) {
                    ...
                    Intent intent = new Intent(ConmonSingleBActivity.this, MainActivity.class);
                    intent.putExtra("flag", "ishome");
                    startActivity(intent);
                } else {
                    Intent intent = new Intent(ConmonSingleBActivity.this, EnvelopeActivity.class);
                    intent.putExtra("type", type);
                    Bundle bundle = new Bundle();
