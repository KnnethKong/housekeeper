...
public class CommonMultiActivity extends AppCompatActivity {
    ...
    private void initData() {
        ...
        data = (ZYEntity) getIntent().getSerializableExtra("data");
        Log.i("stepInformation", step + "--"+type+"-+"+step+"--"+data.getTittleId());
        step = data.getStep();

        if ("4".equals(type) && step == 2) {
            if ("68".equals(data.getTittleId())) {
                isS = true;
                step = step + 2;
            }
        }
//        else if ("3".equals(type) && step == 3) {
//            if ("43".equals(data.getTittleId())) {
//               // isS = true;
//                step = 4;
//            }
//        }
        ...
        if (!"0".equals(type)) {
            ...
            map.put("type", type);
            map.put("secret_key", SPUtils.getString(CommonMultiActivity.this, "secret", ""));
            MyApp.getTingtingApp().getRequestQueue().add(new PlatRequest(CommonMultiActivity.this, Contants.hous_problem, map, null, new Response.Listener<String>() {
                @Override
                public void onResponse(String response) {
                    ...
                    if (code == 200) {
                        ...
                        ToastUtil.longToast(CommonMultiActivity.this, ErrorCodeUtils.getRegisterError(code + ""));
                    }
                }
            }));
        }
    ...
    private void bindDataView() {
        ...
        Log.i("hghInformation22222",
                VipWDUtils.getBeans("step" + step).getType2() +
                        "--" + VipWDUtils.getBeans("step" + step).getProblem().get(0).getType2()
                        + "--" + mZYEntity.getDaList().get(0).getStrId());
        int count = 0;
        if(type.equals("3")){
            count =  VipWDUtils.getBeans("step" + mZYEntity.getStep()).getProblem().size();
        }else{
        ...
        if ("1".equals(VipWDUtils.getBeans("step" + step).getProblem().get(0).getType2()) && "1".equals(VipWDUtils.getBeans("step" + step).getType2())) {
            ...
            if(type.equals("3")){
                setSingleNum(count, VipWDUtils.getBeans("step" + mZYEntity.getStep()).getProblem(), mZYEntity.getDaList().get(0).getStrId());
            }else{
            ...
            if ("66".equals(mZYEntity.getDaList().get(0).getStrId())) {
                ...
                et_single_input_price.setText(SPUtils.getString(CommonMultiActivity.this, "want_money", ""));
...
//                        IMKitService.str = et_single_input_price.getText().toString().trim();
                        SPUtils.saveString(CommonMultiActivity.this, "want_money", et_single_input_price.getText().toString().trim());
        ...
        for (TextView tv : sigleList) {
            if (tv.isSelected()) {
                if (tv.getTag().toString().contains("[")) {
                    ...
                    String content1 = ((TextView) findViewById(R.id.tvY)).getText().toString().substring(1, ((TextView) findViewById(R.id.tvY)).getText().toString().length());
                    ...
                    if (et7.getText() == null || et7.getText().toString().equals("")) {
                        Toast.makeText(CommonMultiActivity.this, getString(R.string.enter_num), Toast.LENGTH_SHORT).show();
                        return "";
                    }
                    if (AppGlobal.getInstance().getLagStr().equals("en")) {
