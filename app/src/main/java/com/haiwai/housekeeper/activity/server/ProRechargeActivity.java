...
/**
 * Created by ihope006 on 2017/4/25.
 * 充值方式筛选
 */
...
public class ProRechargeActivity extends Activity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        top_ordered_bar.setTitle(getString(R.string.wallet_cz));
...
//        tv_wechat_money.setTypeface(MyApp.getTf(), Typeface.NORMAL);
    ...
    private void initHvData() {
        ...
        map.put("secret_key", SPUtils.getString(ProRechargeActivity.this, "secret", ""));
        MyApp.getTingtingApp().getRequestQueue().add(new PlatRequest(ProRechargeActivity.this, Contants.exchange_rate, map, null, new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                ...
                if (code == 200) {//{"status":200,"data":44}
                    ...
                    LoadDialog.closeProgressDialog();
                    ToastUtil.longToast(ProRechargeActivity.this, ErrorCodeUtils.getRegisterError(code + ""));
                }
            }
        }));
    }
    ...
    private void paypal() {
        ...
        PlatRequest request = new PlatRequest(ProRechargeActivity.this, Contants.paypalzhi, map, null, new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                int code = JsonUtils.getJsonInt(response, "status");
                LoadDialog.closeProgressDialog();
                ...
                if (code == 200) {
                    ...
                    Intent intent = new Intent(ProRechargeActivity.this, WebViewActivity.class);
    ...
    private void aliPay() {
        ...
        map.put("secret_key", SPUtils.getString(ProRechargeActivity.this, "secret", ""));
        map.put("login_key", AppGlobal.getInstance().getLoginKey() == null ? "" : AppGlobal.getInstance().getLoginKey());
        Double newDb = mul(Double.valueOf(money), Double.valueOf(hv));
        map.put("money", String.valueOf(newDb));
        PlatRequest request = new PlatRequest(ProRechargeActivity.this, Contants.alipay, map, null, new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                LoadDialog.closeProgressDialog();
                int code = JsonUtils.getJsonInt(response, "status");
    ...
    private void weChat() {
        ...
        Double newDb = mul(Double.valueOf(money), Double.valueOf(hv));
        map.put("money", String.valueOf(newDb));
        PlatRequest request = new PlatRequest(ProRechargeActivity.this, Contants.wxpay, map, null, new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                LoadDialog.closeProgressDialog();
                int code = JsonUtils.getJsonInt(response, "status");
...
//                Toast.makeText(this, "ll_zhifu_bao", Toast.LENGTH_SHORT).show();
            ...
            switch (msg.what) {
                case SDK_PAY_FLAG: {
                    ...
                    if (TextUtils.equals(resultStatus, "9000")) {
                        ...
                        Toast.makeText(ProRechargeActivity.this, getString(R.string.zhifu_s), Toast.LENGTH_SHORT).show();
                        finish();
                    } else {
                        // 该笔订单真实的支付结果，需要依赖服务端的异步通知。
                        Toast.makeText(ProRechargeActivity.this, getString(R.string.zhifu_f), Toast.LENGTH_SHORT).show();
                    }
                    break;
                }
                default:
    ...
    private void alipay(final String orderInfo) {
        Runnable payRunnable = new Runnable() {
            @Override
            public void run() {
                PayTask alipay = new PayTask(ProRechargeActivity.this);
